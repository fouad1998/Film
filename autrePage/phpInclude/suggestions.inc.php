<div class="fd_test container-fluid" style="padding-bottom:50px;">
    ...
    <?
        $reponse =  $bdb->query('SELECT * FROM filmTabe LIMIT 18');
        $i = 0;
        //la premier partie 
    ?>
    ...
    <div class="row"> 
        ...
        <?
            for($i = 0; $i < 12 && ($donnee = $reponse->fetch()); ++$i){
                echo '<div data-toggle="popover" data-trigger="focus" data-content="C est tout simple à faire !" title="khkdlazhkldhkzalh kdhazk hdklhaklz hd" style="margin-bottom:40px;" class="fd_clickToMe fd_elt col-lg-4 col-md-4 col-sm-6 col-xs-6">';
                echo '<img src="'.$donnee['urlImg'].'">';//le lien ajouter
                echo '<label style="display:none;">'.$donnee['bonde'].'</label>';
                echo '<a class="fd_nomFilm" href="'.$donnee['filmUrl'].'"';
                echo 'title="'.$donnee['nom'].'">';
