<?php
	...
	if( $_SESSION['pageOpened'] == 0){ //!empty($_SESSION['pageOpened']) &&
		...
		$reponse->closeCursor();
		$_SESSION['low'] = ceil(($nbrRowOfRequeste) / 18);
	}
	if(empty($_GET['nombre'])){
		$reponse = $bdb->prepare("SELECT * FROM filmTabe LIMIT 0, 18");	
	}
	else{
		$reponse = $bdb->prepare("SELECT * FROM filmTabe LIMIT :debut, 18");
...
?>
	<li class="fd_clickToMe col-lg-4 col-md-4 col-sm-4 col-xs-6" style="cursor:pointer;display:felx;flex-direction:column;height:400px;margin-bottom: 0px;">
		<div class="fd_EltOfFilmLiImg col-12" style="height:90%;">
			<img src="<?php echo $donnee['urlImg']; ?>" alt="" class="fd_liRealImg">
			<label style="display:none;"><? echo $donnee['bonde'];?></label>	
		</div>
		<a href="#" title="nomFilm" style="font-family:arial; padding-top:7px;font-weight: bold;width:100%;font-size:14px;color:black;display:block;text-align:center;"><?php echo $donnee['nom']; ?></a>
	</li>
	<?	};
